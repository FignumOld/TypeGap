...
namespace TypeLite.TsModels {
    ...
    public class TsProperty {
        ...
        public TsProperty(PropertyInfo memberInfo) {
            ...
            this.GenericArguments = propertyType.GetDnxCompatible().IsGenericType ? propertyType.GetDnxCompatible().GetGenericArguments().Select(o => new TsType(o)).ToArray() : new TsType[0];

            this.PropertyType = propertyType.GetDnxCompatible().IsEnum ? new TsEnum(propertyType) : new TsType(propertyType);
        ...
        public TsProperty(FieldInfo memberInfo) {
            ...
            if (memberInfo.DeclaringType.GetDnxCompatible().IsGenericType) {
                var definitionType = memberInfo.DeclaringType.GetDnxCompatible().GetGenericTypeDefinition();
                var definitionTypeProperty = definitionType.GetDnxCompatible().GetProperty(memberInfo.Name);
                if (definitionTypeProperty.PropertyType.IsGenericParameter) {
                    this.PropertyType = TsType.Any;
                } else {
                    this.PropertyType = memberInfo.FieldType.GetDnxCompatible().IsEnum ? new TsEnum(memberInfo.FieldType) : new TsType(memberInfo.FieldType);
                }
            } else {
